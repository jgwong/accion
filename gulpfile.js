 ...
 */
const deployPath = `_deploy/wp-content/themes/idec`; 
...
gulp.task('add', () => {
	return gulp.src('./*', {cwd: '_deploy/'}).pipe(git.add({cwd: '_deploy/'}));
});

gulp.task('commit', () => {
	return gulp.src('./*', {cwd: '_deploy/'}).pipe(git.commit('merge', {cwd: '_deploy/'}));
});

gulp.task('push', (cb) => {
  git.push('wpe', 'main', {cwd: '_deploy/'}, (err) => {
	if (err) throw err;
  });
